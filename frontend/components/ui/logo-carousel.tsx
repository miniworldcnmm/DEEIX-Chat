...
const distributeLogos = (allLogos: LogoCarouselLogo[], columnCount: number): LogoCarouselLogo[][] => {
  if (allLogos.length === 0 || columnCount <= 0) {
    return []
  }
...
const LogoColumn: React.FC<LogoColumnProps> = React.memo(
  ({ logos, index, currentTime, cycleInterval, columnDelay, className, logoClassName }) => {
    const staggerDelay = index * columnDelay // Stagger the start of each column's animation
    // Calculate which logo should be displayed based on the current time
    const adjustedTime =
      (currentTime + staggerDelay) % (cycleInterval * logos.length)
    ...
    return (
      ...
      <motion.div
        className={cn("relative h-14 w-24 overflow-hidden md:h-24 md:w-48", className)}
      ...
      >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <span className="sr-only">{currentLogo.name}</span>
          </motion.div>
        </AnimatePresence>
      </motion.div>
    )
...
function LogoCarousel({
  ...
  const defaultLogos: LogoCarouselLogo[] = useMemo(
    () => [
